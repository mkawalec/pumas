...
These lists include To Do, Review, Doing, and Done.  
...
It is designed to accept an ASCII file that determines the land map on which the simulation takes place.  
The parameters that determine time evolution of both hares and pumas can also be adjusted, although default values are provided.  
The simulation is then initialized, run for a set amount of time steps, and properly finalised.  
Every \texttt{-p} time steps, the population of hares and pumas is printed to an output file using one of the available output methods.  
These outputs can then be visualized, plotted, and analyzed.

\subsection{Input}

The simulation must read in an input land map.  
This land map is intended to represent the two dimensional surface on which the simulation takes place.  
...
The input parameters can be set by the user, otherwise the default values are automatically used.  
...
The Input/Output parameters include the output file names, the output file format, and the frequency at which the progress of the simulation is printed to the terminal.  
...
Parameter groups other than the Generic options can be set from a configuration file. 
It uses the same parameter names as the command line, with a slightly different syntax. 
For syntax example, please see \texttt{contrib/example.cfg}
